function varargout = ConnectPump_HF(varargin)
%% ConnectPump_HF MATLAB code for ConnectPump_HF.fig
%      ConnectPump_HF is an interface for a MicroLynx-controlled pump
...
%
%      ConnectPump_HF was designed by Michelle Maiden using MATLAB's GUIDE
%      
%      ConnectPump_HF must be in the same folder as the following files:
%           ConnectPump_HF.fig, ShdlcDriver.dll, SensorCableDriver.dll, 
%           Test.exe, demo.mat
%
%      ConnectPump_HF, by itself, creates a new ConnectPump_HF or raises the existing
%      singleton*.
%
...
% Last Modified by GUIDE v2.5 16-Jul-2015 12:59:07

%% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @ConnectPump_HF_OpeningFcn, ...
                   'gui_OutputFcn',  @ConnectPump_HF_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before ConnectPump_HF is made visible.
function ConnectPump_HF_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to ConnectPump_HF (see VARARGIN)

% Choose default command line output for ConnectPump_HF
...
% UIWAIT makes ConnectPump_HF wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = ConnectPump_HF_OutputFcn(hObject, eventdata, handles) 
...
% handles    structure with handles and user data (see GUIDATA)
clear handles.s  
    handles.s=instrfind('Type', 'serial', 'Port', 'COM1', 'Tag', '');
    if isempty(handles.s) % If pump not found:
        if strcmp('PCWIN',computer); %If this is a windows machine
            handles.s = serial('COM5'); % Sets the handle to the port
        end
        if strcmo('MACI64',computer) % If this is a mac
...
% Allows ConnectPump_HF to run even if pump is not connected
...
[time, rate] = boundaryDataSoliton(handles.f.mui,handles.f.mue,...
                          handles.f.rhoi,handles.f.rhoe,handles.f.Q0,'small');
...
[time, rate] = boundaryDataDSW(handles.f.mui,handles.f.mue,...
                          handles.f.rhoi,handles.f.rhoe,handles.f.Q0,'h');
handles.progname = 'demo';
    handles.time=time.*60;      %Converts time to s
    handles.rate=mL2uL(rate);   %Converts rate to uL/s
    guidata(hObject,handles);
