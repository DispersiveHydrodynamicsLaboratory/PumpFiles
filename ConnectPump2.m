function varargout = ConnectPump2(varargin)
%% ConnectPump2 MATLAB code for ConnectPump2.fig
%      ConnectPump2 is an interface for a MicroLynx-controlled pump
...
%
%      ConnectPump2 was designed by Michelle Maiden using MATLAB's GUIDE
%      
%      ConnectPump2 must be in the same folder as the following files:
%           ConnectPump2.fig, ShdlcDriver.dll, SensorCableDriver.dll, 
%           Test.exe, demo.mat
%
%      ConnectPump2, by itself, creates a new ConnectPump2 or raises the existing
%      singleton*.
%
...
% Last Modified by GUIDE v2.5 06-Sep-2016 18:06:13

%% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @ConnectPump2_OpeningFcn, ...
                   'gui_OutputFcn',  @ConnectPump2_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before ConnectPump2 is made visible.
function ConnectPump2_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to ConnectPump2 (see VARARGIN)

% Choose default command line output for ConnectPump2
...
% UIWAIT makes ConnectPump2 wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = ConnectPump2_OutputFcn(hObject, eventdata, handles) 
...
clear handles.s  
     ...
     if strcmp('MACI64',computer) % If this is a mac
        ...
        if strcmp('MACI64', computer) % If this is a mac
            handles.s=instrfind('Type', 'serial', 'Port', '/dev/tty.usbserial1', 'Tag', '');
            handles.s = serial('/dev/tty.usbserial1');
        end
...
% Allows ConnectPump2 to run even if pump is not connected
...
backgroundRate=get(hObject,'String');
    ...
    if get(handles.pumpon,'Value')==0
         uLynx(handles.s,handles.t,'DECL = 50',1,handles.print_on,handles)
...
[time, rate] = boundaryDataSoliton(handles.f.mui,handles.f.mue,...
                          handles.f.rhoi,handles.f.rhoe,handles.f.Q0,'small',handles.f.pump_type);
...
ylabel(handles.proginput,'Rate (mL/min)');
handles.ax=axis;
ConnectPump('makeprog_Callback',hObject,eventdata,handles);
ConnectPump('runprog_Callback',hObject,eventdata,handles);
...
% handles    structure with handles and user data (see GUIDATA)
[time, rate] = boundaryDataSoliton(handles.f.mui,handles.f.mue,...
                          handles.f.rhoi,handles.f.rhoe,handles.f.Q0,'large',handles.f.pump_type);
handles.progname = 'demo';
    handles.time=time.*60;      %Converts time to s
    handles.rate=mL2uL(rate);   %Converts rate to uL/s
    guidata(hObject,handles);
...
[time, rate] = boundaryDataSolitonSoliton_same(handles.f.mui,handles.f.mue,...
                          handles.f.rhoi,handles.f.rhoe,handles.f.Q0,handles.f.pump_type);
